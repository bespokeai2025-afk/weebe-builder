  ...
  <Html lang="en" dir="ltr">
    ...
    <Body style={main}>
      <Container style={container}>
        ...
        <Text style={text}>
          A customer just upgraded their plan on <strong>{SITE_NAME}</strong>.
          Review the details below to see what new features, agents, or
          requirements may need provisioning for them.
        </Text>
        <Text style={row}><strong>Customer:</strong> {customerEmail ?? 'unknown'}</Text>
        <Text style={row}><strong>Previous plan:</strong> {previousPlan ?? '—'}</Text>
        <Text style={row}><strong>New plan:</strong> {newPlan ?? '—'}</Text>
        <Text style={row}><strong>Subscription ID:</strong> {stripeSubscriptionId ?? '—'}</Text>
        <Text style={row}><strong>Customer ID:</strong> {stripeCustomerId ?? '—'}</Text>
