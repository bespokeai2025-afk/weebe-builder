...
export async function listEventTypes(apiKey: string): Promise<CalcomEventType[]> {
  const data = await calFetch<{ eventTypeGroups?: Array<{ eventTypes: CalcomEventType[] }> } | CalcomEventType[]>(
    apiKey,
    "/event-types",
  );
  if (Array.isArray(data)) return data;
  const groups = (data as { eventTypeGroups?: Array<{ eventTypes: CalcomEventType[] }> }).eventTypeGroups ?? [];
