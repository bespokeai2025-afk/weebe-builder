...
export function importAgentJson(raw: string): {
  ...
  const nodes: FlowNode[] = rawNodes.map((rn, idx) => {
    ...
    const nodeData: FlowNodeData = {
      ...
      transferMode:
        rn.transfer_destination?.type === "dynamic_variable" || rn.transfer_destination?.type === "inferred"
          ? "dynamic"
          : "static",
      transferDynamicVariable: rn.transfer_destination?.prompt ?? rn.transfer_destination?.variable_name,
      transferType: rn.transfer_option?.type === "warm_handoff" ? "agentic_warm_transfer" : rn.transfer_option?.type,
      ...
      transferAgentVersion: rn.transfer_option?.agentic_transfer_config?.transfer_agent?.agent_version,
      transferTimeoutSec:
