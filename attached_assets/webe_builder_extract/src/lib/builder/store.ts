    ...
    (set, get) => ({
      ...
      addBookingNode: (position) => {
        ...
        dialogue: [
          ...
          "2. `email` — a valid email address. Ask: \"What email should I send the confirmation to?\" Then spell it back letter-by-letter to confirm (e.g. \"j-o-h-n at gmail dot com\"). Do NOT proceed if you are unsure of the email — re-ask until confirmed. Never invent or guess an email.",
          "3. `start` — the ISO 8601 start time of the slot the caller picked (you get this from check_availability).",
          "4. `timezone` — IANA timezone (e.g. \"America/New_York\"). Default to the workspace timezone if the caller does not specify one.",
          "5. `phone` — optional, but ask if email collection is failing: \"Would you prefer I text the confirmation to a phone number instead?\"",
          "",
          "## How to handle it",
          "1. Greet and ask what they'd like to book.",
          "2. Collect name and email per the rules above. Confirm the email by spelling it back.",
          "3. Ask for preferred day and rough time window; confirm their timezone.",
          "4. Call `check_availability` with the requested date range to fetch open slots.",
          "5. Read 2–3 nearby options back in natural language (e.g. \"Tuesday at 2pm or Wednesday at 10am\").",
          "6. Once they pick a slot, call `book_appointment` with ALL of: name, email (confirmed), start (ISO from the slot), timezone, and phone if collected.",
          "7. Confirm the booking out loud and tell them a confirmation email is on the way.",
          "8. For reschedules/cancellations, ask for the booking reference or the email used, then call `reschedule_appointment` or `cancel_appointment`.",
          "",
          "## Rules",
          "- NEVER call `book_appointment` without a confirmed email OR phone — the API will reject the booking.",
      ...
      loadFlow: (data) =>
        set({
          ...
          currentAgentRowId: data.agentRowId === undefined ? get().currentAgentRowId : data.agentRowId,
          selectedNodeId: null,
          flowVersion: get().flowVersion + 1,
        }),
