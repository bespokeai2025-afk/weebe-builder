...
function coerceTransferRingMs(value: unknown, label: string): number | undefined {
  ...
  if (!Number.isFinite(raw)) throw new Error(`Call Transfer "${label}" has an invalid ring duration.`);
...
function cleanSipHeaders(value: unknown, label: string): Record<string, string> {
  ...
  for (const [key, val] of Object.entries(raw as Record<string, unknown>)) {
    ...
    if (!/^x-/i.test(k) && !/^user-to-user$/i.test(k)) {
      throw new Error(`Call Transfer "${label}" has invalid SIP header "${k}". Header names must start with X- or be User-To-User.`);
    }
    out[k] = v;
  }
  return out;
...
function validateTransferExtension(value: unknown, label: string): string | undefined {
  ...
  throw new Error(`Call Transfer "${label}" has an invalid extension. Use digits, * or #, for example 123#.`);
}
...
function mapNode(n: FlowNode, edges: FlowEdge[]): Record<string, unknown> & { id: string } {
  ...
  switch (d.kind) {
    ...
    case "call_transfer": {
      ...
      const isDynamic =
        (d.transferMode ??
          (rawDest.type === "dynamic_variable" || rawDest.type === "inferred" ? "dynamic" : "static")) ===
        "dynamic";

      const ignoreE164 =
        d.ignoreE164Validation ?? (raw.ignore_e164_validation as boolean) ?? false;

      let transfer_destination: Record<string, unknown>;
      if (isDynamic) {
        const v = String(d.transferDynamicVariable ?? rawDest.prompt ?? rawDest.variable_name ?? "").trim();
        if (!v) {
          throw new Error(
            `Call Transfer "${d.label}" is set to dynamic but has no variable name.`,
          );
        }
        ...
        transferType === "cold_transfer"
          ? {
              ...
              ...(d.sipTransferMethod || rawOpt.cold_transfer_mode
                ? { cold_transfer_mode: d.sipTransferMethod ?? (rawOpt.cold_transfer_mode as string) }
                : {}),
            }
          : {
              ...rawOpt,
      ...
      if (transferType === "warm_transfer") {
        ...
        const publicHandoff =
          buildHandoffOption(d.warmHandoffPrompt) ??
          buildHandoffOption(rawOpt.public_handoff_option) ??
          { type: "prompt", prompt: "Transferring you to a human agent now." };
        transfer_option.public_handoff_option = publicHandoff;
        const privateHandoff = buildHandoffOption(d.privateHandoffPrompt) ?? buildHandoffOption(rawOpt.private_handoff_option);
      ...
      if (transferType === "agentic_warm_transfer") {
        const rawConfig = (rawOpt.agentic_transfer_config as Record<string, unknown> | undefined) ?? {};
        ...
        if (!agentId) {
          throw new Error(`Call Transfer "${d.label}" uses Agentic Warm Transfer but has no transfer agent ID.`);
        }
        transfer_option.on_hold_music = rawOpt.on_hold_music ?? "ringtone";
        transfer_option.public_handoff_option =
          buildHandoffOption(d.warmHandoffPrompt) ??
          buildHandoffOption(rawOpt.public_handoff_option) ??
          { type: "prompt", prompt: "Transferring you to another agent now." };
      ...
      const cleanedSipHeaders = cleanSipHeaders(d.customSipHeaders ?? raw.custom_sip_headers, d.label);
      ...
      return orderNode(
        stripUndefined({
          ...
          transfer_option,
          speak_during_execution:
            (raw.speak_during_execution as boolean | undefined) ?? false,
    ...
    case "agent_transfer": {
      ...
      const instruction =
        raw.instruction ?? { type: "prompt", text: "" };
      return orderNode({
        ...base,
        webhook_setting:
        ...
        keep_current_voice:
          d.agentSwapKeepCurrentVoice ??
          (raw.keep_current_voice as boolean | undefined) ??
          false,
        type: "agent_swap",
    ...
    case "sms": {
      ...
      const success_edge: FlowEdge =
        rawSuccess ??
        (successFromEdges
          ? { ...successFromEdges, transition_condition: { type: "prompt", prompt: "Sent successfully" } }
          : { id: `success_edge-${n.id}`, transition_condition: { type: "prompt", prompt: "Sent successfully" } });
      const failed_edge: FlowEdge =
        rawFailed ??
        (failedFromEdges
          ? { ...failedFromEdges, transition_condition: { type: "prompt", prompt: "Failed to send" } }
          : { id: `failed_edge-${n.id}`, transition_condition: { type: "prompt", prompt: "Failed to send" } });
      return orderNode({
        ...base,
        type: "sms",
    ...
    case "extract_variable": {
      const rawVar = raw.variable as { name?: string; description?: string; type?: string } | undefined;
      const rawVars = (raw.variables as Array<{ name?: string; description?: string; type?: string }> | undefined);
      const variables = rawVars && rawVars.length
        ? rawVars.map((v) => ({
            name: v.name ?? "var",
            description: v.description ?? "",
            type: v.type ?? "string",
