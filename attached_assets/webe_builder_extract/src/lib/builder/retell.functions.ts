 ...
 */
async function maybeBuildBookingToolsForUser(userId: string) {
 ...
 */
function transferSchemaMismatches(expected: Record<string, unknown>, actual: Record<string, unknown>) {
  const HARD = ["type", "transfer_destination"] as const;
  const SOFT = ["transfer_option", "ignore_e164_validation", "custom_sip_headers", "extension_dial_string", "cold_transfer_mode", "show_transferee_as_caller", "transfer_ring_duration_ms"] as const;
  const hard: string[] = [];
  const soft: string[] = [];
  for (const f of HARD) {
    if (expected[f] !== undefined && JSON.stringify(expected[f]) !== JSON.stringify(actual[f])) hard.push(f);
  }
  for (const f of SOFT) {
    if (expected[f] !== undefined && JSON.stringify(expected[f]) !== JSON.stringify(actual[f])) soft.push(f);
  }
  return { hard, soft };
}
...
export const deployAgentToRetell = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const mode: "create" | "update" =
      data.mode ?? (data.agentId ? "update" : "create");
    ...
    const bookingTools = bookingEnabled
      ? await maybeBuildBookingToolsForUser(context.userId)
      : null;
    ...
    if (bookingEnabled && data.calToolOverrides?.length) {
      ...
      for (const ov of data.calToolOverrides) {
        ...
        const defaultNames: Record<string, string> = {
          check_availability: "check_availability_cal",
          book_appointment: "book_appointment_cal",
          reschedule_appointment: "reschedule_appointment_cal",
          cancel_appointment: "cancel_appointment_cal",
        };
    ...
    if (calendarConnected || perNodeTools.length > 0) {
      const bookingAnalysisFields = [
        {
          ...
          examples: ["Booked John Smith for a dental cleaning on 2026-06-10 at 14:00. Requested morning reminder call."],
        },
        {
          type: "boolean",
      ...
      if (!agent.webhook_url) {
        agent.webhook_url = `${PUBLIC_BASE_URL_FOR_WEBHOOK}/api/public/retell/call-analyzed`;
      }
    }
...
export const cloneCustomVoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!res.ok) {
      const message =
        (parsed.message as string | undefined) || text || res.statusText;
...
export const listRetellPhoneNumbers = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { productionApiKey?: string; agentRowId?: string } | undefined) => input ?? {})
  .handler(async ({ data, context }) => {
...
export const cloneRetellAgentForDeploy = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const prodKey = (data.productionApiKey ?? "").trim();
    if (!prodKey) throw new Error("A production Retell API key is required");
    ...
    // Auto-attach booking tools if this user has Cal.com connected.
    const bookingTools = await maybeBuildBookingToolsForUser(context.userId);
    const calendarConnected = bookingTools !== null;
    if (bookingTools) {
