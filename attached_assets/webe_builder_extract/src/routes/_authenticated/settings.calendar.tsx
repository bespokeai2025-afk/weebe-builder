...
export const Route = createFileRoute("/_authenticated/settings/calendar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Connect Google Calendar through Cal.com for your voice agents." },
    ],
  }),
    ...
    <main className="min-h-screen bg-background flex flex-col">
      <header className="border-b shrink-0">
        <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
          <Link to="/my-agents" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-8 space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Connect your Google Calendar through Cal.com. Your voice agents book through your backend — never directly into Google.
          </p>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Paste a Cal.com API key (Settings → Developer → API keys). To connect Google Calendar, link it from your Cal.com account first.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-2">
              <Label htmlFor="apiKey">Cal.com API key {hasKey && <span className="text-xs text-muted-foreground">(leave blank to keep current)</span>}</Label>
              <Input
                id="apiKey"
                type="password"
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <div className="grid gap-2">
                ...
                <Input id="tz" value={timezone} onChange={(e) => setTimezone(e.target.value)} placeholder="Europe/London" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="buf">Buffer (min)</Label>
                <Input id="buf" type="number" min={0} value={buffer} onChange={(e) => setBuffer(Number(e.target.value))} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="not">Min notice (hours)</Label>
                <Input id="not" type="number" min={0} value={minNotice} onChange={(e) => setMinNotice(Number(e.target.value))} />
              </div>
            </div>
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                Used when an agent doesn't have its own event type override. Sync first to see available IDs below.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild variant="ghost">
                <a href="https://app.cal.com/apps/google-calendar" target="_blank" rel="noopener noreferrer">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Pulled from Cal.com. Toggle which to read for availability and where to write bookings.</CardDescription>
          </CardHeader>
          <CardContent>
            {calsQ.isLoading ? (
              <div className="text-sm text-muted-foreground flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
            ) : (calsQ.data ?? []).length === 0 ? (
              <p className="text-sm text-muted-foreground">No calendars yet. Connect Google in Cal.com, then sync.</p>
            ) : (
                ...
                {(calsQ.data ?? []).map((c) => (
                  <div key={c.id} className="py-3 flex items-center gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground truncate">{c.email} · {c.provider}{c.read_only ? " · read-only" : ""}</div>
                    </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Synced from Cal.com. Disable any you don't want agents to book.</CardDescription>
          </CardHeader>
          <CardContent>
            {etsQ.isLoading ? (
              <div className="text-sm text-muted-foreground flex items-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
                  ...
                  <div key={et.id} className="py-3 flex items-center gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        ID {et.calcom_event_type_id} · {et.length_minutes} min{et.slug ? ` · /${et.slug}` : ""}
                      </div>
                    </div>
                    <Button
                      size="sm"
                  ...
                  <div key={b.id} className="py-3 flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      <div className="text-sm font-medium truncate">
                        {b.attendee_name} <span className="text-muted-foreground">· {b.attendee_email}</span>
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {new Date(b.start_at).toLocaleString()} → {new Date(b.end_at).toLocaleTimeString()}
                      </div>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded ${b.status === "cancelled" ? "bg-destructive/10 text-destructive" : "bg-green-500/10 text-green-700 dark:text-green-400"}`}>
                      {b.status}
                    </span>
                  </div>
                ))}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add these custom functions to your voice agent to enable booking via voice.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <p className="text-muted-foreground">Endpoint base: <code className="px-1 py-0.5 rounded bg-muted">https://webespokegenbuilder.lovable.app/api/public/booking</code></p>
            <ul className="list-disc pl-5 space-y-1">
              <li><code>POST /availability</code> — body <code>{`{ agent_id, start_date, end_date, timezone? }`}</code></li>
              <li><code>POST /book</code> — body <code>{`{ agent_id, start, name, email, phone?, notes?, call_id? }`}</code></li>
              <li><code>POST /cancel</code> — body <code>{`{ booking_id, reason? }`}</code></li>
            </ul>
            <p className="text-xs text-muted-foreground">
              Header <code>x-signature</code>: HMAC-SHA256 hex of the raw body using your webhook secret.
              If the secret isn't set, signature checks are skipped (dev only).
            </p>
          </CardContent>
        </Card>
      </div>
