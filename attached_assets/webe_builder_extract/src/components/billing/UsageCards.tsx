...
function Card({ icon, label, value, foot, progress }: {
...
export function UsageCards({
  plan, activeAgents, minutesUsed, callsMade, cycleCostCents, cycleStart, nextBillingAt,
}: Props) {
  ...
  const cycleStartFmt = new Date(cycleStart).toLocaleDateString("en-GB", { day: "numeric", month: "short" });
  const nextBillingFmt = nextBillingAt
    ? new Date(nextBillingAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })
    : "—";
      ...
      <Card
        ...
        value={
          <>
            ...
            {!unlimited && <span className="ml-1 text-sm font-normal text-muted-foreground">/ {included}</span>}
          </>
        }
