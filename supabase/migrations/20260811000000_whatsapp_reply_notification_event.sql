-- Adds the "whatsapp_reply_received" notification event key to the
-- workspace_notification_settings event_key check constraint.
-- Applied to the live database on 2026-08-11 via the Management API.
...
ALTER TABLE workspace_notification_settings
  ...
  CHECK (event_key = ANY (ARRAY[
    ...
    'lead_created','whatsapp_reply_received'
  ]));
