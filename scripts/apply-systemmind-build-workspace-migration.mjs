/**
 * One-time migration: creates the SystemMind Build Workspace tables.
 *   systemmind_build_sessions
 *   systemmind_build_versions
 *   systemmind_build_messages
 *   systemmind_usage_events
 *   cost_engine_systemmind
 * Plus additive provenance columns on workspace_workflows.
 *
 * Run: node scripts/apply-systemmind-build-workspace-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260812000000_systemmind_build_workspace.sql"),
  "utf8",
);
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-build-workspace-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-build-workspace-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-build-workspace-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    // The workspace_workflows columns are additive + idempotent; if the tables
    // exist the whole migration ran (single transaction via Management API).
    console.log("✅ All SystemMind Build Workspace tables already exist — nothing to do.");
    process.exit(0);
  }

  console.log("One or more Build Workspace tables missing — proceeding with migration.\n");
} else {
  console.log("[sm-build-workspace-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind Build Workspace migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-build-workspace-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-build-workspace-migration] Management API network error:", err?.message);
  }
}
