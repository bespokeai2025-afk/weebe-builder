...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20261002000000_call_variables_tool_calls.sql"),
  "utf8",
);
...
if (supabase) {
  const hasTurns = await columnExists(supabase, "calls", "collected_variables");
  const hasFlag = await columnExists(supabase, "calls", "tool_calls");
  if (hasTurns && hasFlag) {
    console.log("✅ Already applied — calls.collected_variables and calls.tool_calls both present.");
    process.exit(0);
  }
  console.log(
    `Applying… (collected_variables: ${hasTurns ? "present" : "missing"}, tool_calls: ${hasFlag ? "present" : "missing"})`,
  );
}
...
if (supabase) {
  ...
  if (!hasTurns || !hasFlag) {
    console.error(
      `❌ API reported success but verification failed (collected_variables: ${hasTurns}, tool_calls: ${hasFlag}).`,
    );
    process.exit(1);
  }
}

console.log("✅ calls.collected_variables + calls.tool_calls applied and verified.");
process.exit(0);
