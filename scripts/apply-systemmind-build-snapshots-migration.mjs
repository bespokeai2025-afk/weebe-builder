/**
 * One-time migration: creates the SystemMind Build Workspace protection table.
 *   systemmind_build_snapshots
 *
 * Run: node scripts/apply-systemmind-build-snapshots-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260713000000_systemmind_build_snapshots.sql"),
  "utf8",
);
...
const TABLES = ["systemmind_build_snapshots"];
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-build-snapshots-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-build-snapshots-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-build-snapshots-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ systemmind_build_snapshots already exists — nothing to do.");
    process.exit(0);
  }

  console.log("Build snapshots table missing — proceeding with migration.\n");
} else {
  console.log("[sm-build-snapshots-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind Build snapshots migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-build-snapshots-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-build-snapshots-migration] Management API network error:", err?.message);
  }
}
