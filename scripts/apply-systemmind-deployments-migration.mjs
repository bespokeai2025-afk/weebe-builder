/**
 * One-time migration: creates the SystemMind Deployment Orchestrator tables.
 *   systemmind_deployments
 *   systemmind_deployment_approvals
 *
 * Run: node scripts/apply-systemmind-deployments-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260719000000_systemmind_deployments.sql"),
  "utf8",
);
...
const TABLES = ["systemmind_deployments", "systemmind_deployment_approvals"];
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-deployments-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-deployments-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-deployments-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ systemmind_deployments tables already exist — nothing to do.");
    process.exit(0);
  }

  console.log("Deployment tables missing — proceeding with migration.\n");
} else {
  console.log("[sm-deployments-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind deployments migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-deployments-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-deployments-migration] Management API network error:", err?.message);
  }
}
