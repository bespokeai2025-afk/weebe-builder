/**
 * One-time migration: AccountsMind Config Builder + Onboarding & Health tables.
 *   accountsmind_field_defs, accountsmind_stat_defs, accountsmind_widget_defs,
 *   accountsmind_field_values, workspace_setup_checklists, workspace_health_runs
 *
 * Run: node scripts/apply-accountsmind-config-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260815000000_accountsmind_config_onboarding_health.sql"),
  "utf8",
);
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[am-config-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[am-config-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[am-config-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ All AccountsMind config/onboarding/health tables already exist — nothing to do.");
    process.exit(0);
  }

  console.log("One or more tables missing — proceeding with migration.\n");
} else {
  console.log("[am-config-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    console.warn("[am-config-migration] Management API network error:", err?.message);
  }
}
