/**
 * One-time migration: creates the SystemMind agent scanner & dynamic variable
 * engine tables:
 ...
 *
 * Run: node scripts/apply-systemmind-variable-engine-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260730000000_systemmind_variable_engine.sql"),
  "utf8",
);
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-variable-engine-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-variable-engine-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-variable-engine-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ SystemMind variable engine tables already exist — nothing to do.");
    process.exit(0);
  }

  console.log("Variable engine tables missing — proceeding with migration.\n");
} else {
  console.log("[sm-variable-engine-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind variable engine migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-variable-engine-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-variable-engine-migration] Management API network error:", err?.message);
  }
}
