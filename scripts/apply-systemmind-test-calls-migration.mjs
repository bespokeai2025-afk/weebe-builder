/**
 * One-time migration: creates the SystemMind Test Call Validation Loop history table.
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260714000000_systemmind_test_calls.sql"),
  "utf8",
);
...
const TABLES = ["systemmind_test_calls"];
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-test-calls-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-test-calls-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-test-calls-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ systemmind_test_calls already exists — nothing to do.");
    process.exit(0);
  }

  console.log("Test calls table missing — proceeding with migration.\n");
} else {
  console.log("[sm-test-calls-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind test calls migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-test-calls-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-test-calls-migration] Management API network error:", err?.message);
  }
}
