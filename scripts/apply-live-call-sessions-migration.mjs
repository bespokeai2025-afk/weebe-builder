/**
 * One-time migration: creates the live_call_sessions table.
 * Run: node scripts/apply-live-call-sessions-migration.mjs
 *
 * Optional env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260724000000_live_call_sessions.sql"),
  "utf8",
);
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  if (!checkErr) {
    console.log("✅ live_call_sessions table already exists — nothing to do.");
    process.exit(0);
  }

  if (
    checkErr.code !== "PGRST205" &&
    !checkErr.message?.includes("relation") &&
    !checkErr.message?.includes("live_call_sessions")
  ) {
...
if (projectRef && mgmtToken) {
  ...
  if (res.ok) {
    console.log("✅ live_call_sessions migration applied via Management API!");
    refreshSchemaMap();
    process.exit(0);
  }
