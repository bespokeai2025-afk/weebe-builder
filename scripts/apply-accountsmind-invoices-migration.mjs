 ...
 * Run: node scripts/apply-accountsmind-invoices-migration.mjs
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260722000000_accountsmind_invoices.sql"),
  "utf8",
);
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ accountsmind invoices tables created via Management API!");
      process.exit(0);
    }
    console.warn("[invoices-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[invoices-migration] Management API network error:", err?.message);
  }
}
