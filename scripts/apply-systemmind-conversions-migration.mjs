/**
 * One-time migration: creates the SystemMind Legacy Logic Converter lineage table.
 *   systemmind_conversions
 *
 * Run: node scripts/apply-systemmind-conversions-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260718000000_systemmind_conversions.sql"),
  "utf8",
);
...
const TABLES = ["systemmind_conversions"];
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-conversions-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-conversions-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-conversions-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ systemmind_conversions already exists — nothing to do.");
    process.exit(0);
  }

  console.log("Conversions table missing — proceeding with migration.\n");
} else {
  console.log("[sm-conversions-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind conversions migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-conversions-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-conversions-migration] Management API network error:", err?.message);
  }
}
