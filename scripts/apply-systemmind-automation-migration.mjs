/**
 * One-time migration: creates the SystemMind Automation Layer tables.
 *   systemmind_runs
 *   systemmind_generated_actions
 *   systemmind_audit_logs
 *
 * Run: node scripts/apply-systemmind-automation-migration.mjs
 *
 * Env vars:
...
const SQL = readFileSync(
  resolve(__dir, "../supabase/migrations/20260806000000_systemmind_automation_layer.sql"),
  "utf8",
);
...
if (SUPABASE_URL && SERVICE_KEY) {
  ...
  try {
    ...
    console.warn("[sm-automation-migration] Failed to create Supabase client:", err?.message);
    printFallback();
    process.exit(0);
  }
  ...
  for (const table of TABLES) {
    try {
      ...
      if (error) {
        ...
        console.warn(`[sm-automation-migration] Unexpected check error on ${table}: ${error.message}`);
        console.log("Skipping migration check to avoid blocking post-merge.");
        process.exit(0);
      }
    } catch (err) {
      console.warn(`[sm-automation-migration] Network error checking ${table}:`, err?.message);
      console.log("Skipping migration check to avoid blocking post-merge.");
      process.exit(0);
    }
  }
  ...
  if (allPresent) {
    console.log("✅ All SystemMind Automation Layer tables already exist — nothing to do.");
    process.exit(0);
  }

  console.log("One or more automation tables missing — proceeding with migration.\n");
} else {
  console.log("[sm-automation-migration] No Supabase credentials found — printing SQL for manual apply.");
}
...
if (projectRef && mgmtToken) {
  ...
  try {
    ...
    if (res.ok) {
      console.log("✅ SystemMind Automation Layer migration applied via Management API!");
      refreshSchemaMap();
      process.exit(0);
    }
    console.warn("[sm-automation-migration] Management API error:", JSON.stringify(json));
  } catch (err) {
    console.warn("[sm-automation-migration] Management API network error:", err?.message);
  }
}
