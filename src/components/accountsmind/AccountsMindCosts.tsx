  ...
  return (
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground dark:text-gray-400 mt-0.5">Current month — aggregated from usage logs</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {catEntries.map(([cat, cents]) => (
          <div key={cat} className="bg-card border border-border dark:bg-gray-900 dark:border-gray-800 rounded-xl p-4">
            <div className="flex items-center gap-2 mb-2">
              <div className={cn("w-2 h-2 rounded-full", colorFor(cat))} />
              <span className="text-xs text-muted-foreground dark:text-gray-400 capitalize">{cat}</span>
            </div>
            <div className="text-lg font-bold text-foreground dark:text-white">
              £{(cents / 100).toFixed(2)}
            </div>
            <div className="text-[10px] text-muted-foreground dark:text-gray-500 mt-0.5">
      ...
      <div className="bg-card border border-border dark:bg-gray-900 dark:border-gray-800 rounded-xl p-4 flex items-center justify-between">
        <span className="text-sm text-muted-foreground dark:text-gray-400">Total provider spend this month</span>
        <span className="text-xl font-bold text-foreground dark:text-white">£{(total / 100).toFixed(2)}</span>
      </div>

      {/* Provider breakdown table */}
      <div className="bg-card border border-border dark:bg-gray-900 dark:border-gray-800 rounded-xl p-4">
        <h3 className="text-sm font-semibold text-foreground dark:text-white mb-3">Provider Breakdown</h3>
        {(data as any[]).length === 0 && !isLoading && (
          <p className="text-xs text-muted-foreground dark:text-gray-500">No provider usage recorded for this month.</p>
        )}
        <div className="space-y-1">
          {(data as any[]).map((r: any, i: number) => {
            const barPct = total > 0 ? (r.costCents / total) * 100 : 0;
            return (
              <div key={i} className="flex items-center gap-3 py-1.5 border-b border-border/50 dark:border-gray-800/50 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-foreground/80 dark:text-gray-300">
                      <span className="text-muted-foreground dark:text-gray-500 capitalize">{r.category}</span>
                      {" / "}
                      <span className="text-foreground dark:text-white">{r.provider}</span>
                    </span>
                    <span className="text-foreground/80 dark:text-gray-300 font-medium">£{(r.costCents / 100).toFixed(2)}</span>
                  </div>
                  <div className="w-full bg-muted dark:bg-gray-800 rounded-full h-1">
                    <div
