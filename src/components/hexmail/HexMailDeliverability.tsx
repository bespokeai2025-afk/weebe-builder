...
function WebhookPanel() {
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-border dark:border-white/[0.06] bg-muted/20 dark:bg-white/[0.02] p-4 flex items-center gap-2 text-xs text-muted-foreground/50">
  ...
  if (wh?.noApiKey) {
    return (
      <div className="rounded-xl border border-border dark:border-white/[0.06] bg-muted/20 dark:bg-white/[0.02] p-4 flex items-center justify-between gap-4">
        ...
        <Link to="/hexmail/settings" className="shrink-0 rounded-lg border border-border dark:border-white/[0.08] bg-muted/20 dark:bg-white/[0.03] px-3 py-1.5 text-xs hover:bg-muted/50 dark:hover:bg-white/[0.06] transition-colors">
          Add API Key
        </Link>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-border dark:border-white/[0.08] bg-muted/20 dark:bg-white/[0.02] p-4 flex items-center justify-between gap-4">
      <div className="flex items-center gap-3">
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-6xl">
      <div className="flex items-start justify-between">
        ...
        <button
          onClick={() => refetch()}
          className="flex items-center gap-1.5 rounded-lg border border-border dark:border-white/[0.08] bg-muted/20 dark:bg-white/[0.03] px-3 py-1.5 text-xs text-muted-foreground hover:bg-muted/50 dark:hover:bg-white/[0.06] transition-colors"
        >
      ...
      {/* Health Score */}
      <div className="rounded-xl border border-border dark:border-white/[0.08] bg-gradient-to-br from-muted/20 dark:from-white/[0.03] to-transparent p-6 flex flex-col sm:flex-row items-start sm:items-center gap-6">
        <div className="flex flex-col gap-1 min-w-[120px]">
          <span className="text-xs text-muted-foreground/60 uppercase tracking-widest">Domain Health Score</span>
          {isLoading
            ? <div className="h-10 w-20 animate-pulse rounded-lg bg-muted/40 dark:bg-white/[0.06]" />
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[...Array(8)].map((_, i) => (
            <div key={i} className="h-24 animate-pulse rounded-xl bg-muted/20 dark:bg-white/[0.03]" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {QUICK_LINKS.map((link) => (
            <Link key={link.to} to={link.to}
              className="group flex items-center justify-between rounded-xl border border-border dark:border-white/[0.06] bg-muted/20 dark:bg-white/[0.02] p-4 hover:border-border dark:hover:border-white/[0.12] hover:bg-muted/30 dark:hover:bg-white/[0.04] transition-all"
            >
              <div className="flex items-center gap-3">
      ...
      {!isLoading && (stats?.totalDomains ?? 0) === 0 && (
        <div className="rounded-xl border border-dashed border-border dark:border-white/[0.10] p-10 text-center flex flex-col items-center gap-3">
