    ...
    <div className="flex flex-col gap-6 p-6 max-w-4xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="sm:col-span-1">
              ...
              <Input value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder="hello@yourdomain.com" className="bg-muted/60 border-border" />
            </div>
            <div>
      ...
      {isLoading ? (
        <div className="flex flex-col gap-3">
          {[...Array(3)].map((_, i) => <div key={i} className="h-16 animate-pulse rounded-xl bg-muted/60" />)}
        </div>
      ) : mailboxes.length === 0 ? (
                  ...
                  {editId === m.id ? (
                    <div className="flex items-center justify-end gap-1">
                      <Input type="number" value={editLimit} onChange={(e) => setEditLimit(Number(e.target.value))}
                        className="w-20 h-6 text-xs text-right bg-muted/60 border-border" />
                ...
                <div className="col-span-2 flex items-center justify-end gap-2">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <select value={m.status} onChange={(e) => updateMut.mutate({ mailboxId: m.id, status: e.target.value })}
                      className="h-6 text-[10px] rounded border border-border bg-muted/60 px-1">
      ...
      <div className="rounded-xl border border-border bg-amber-500/[0.03] p-4 flex items-start gap-3">
