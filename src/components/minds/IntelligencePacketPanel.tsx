      ...
      {expanded && !compact && (
        <div className="border-t border-sky-500/10 px-3 py-3 space-y-3">
          ...
          <div>
            ...
            {devOpen && (
              <pre className="mt-1.5 text-[10px] text-muted-foreground bg-muted/40 rounded-lg p-2 overflow-x-auto border border-border max-h-64">
...
export function ApprovalDialog({ meta, packet, readinessState, onConfirm, onCancel, busy }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onCancel}>
      <div
        className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-xl border border-border bg-[hsl(var(--card))] shadow-2xl"
      ...
      >
        ...
        <div className="px-5 py-3.5 border-t border-border flex items-center justify-end gap-2">
          <button
            onClick={onCancel}
            disabled={busy}
            className="rounded-lg border border-border bg-muted/60 px-3.5 py-1.5 text-[11px] text-muted-foreground hover:text-foreground transition-all disabled:opacity-40"
          >
            Cancel
          </button>
          <button
