...
import { normalizeBuilderSpeechMode } from "@/lib/voice/graph/speech-mode.shared";
        ...
        return (
          <div key={t.id} className={cn(NODE_ITEM_CLASS, "group/row items-start")}>
            ...
            <div className="nodrag nopan mt-0.5 flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover/row:opacity-100">
              <button
                type="button"
                className="rounded p-0.5 text-slate-300 hover:bg-muted hover:text-white"
...
function ConversationStyleNode({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  const instructionType = normalizeBuilderSpeechMode(
    nodeData.instructionType ?? (kind === "wait" || kind === "begin" ? "static_text" : "prompt"),
  );
...
function SimpleNode({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  const instructionType = normalizeBuilderSpeechMode(nodeData.instructionType ?? "prompt");
