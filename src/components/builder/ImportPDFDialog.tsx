        ...
        {phase === "idle" && (
          <div className="space-y-3">
            ...
            <a
              ...
              className="flex items-center justify-center gap-1.5 w-full rounded-md border border-border dark:border-white/[0.08] bg-muted/40 dark:bg-white/[0.02] px-3 py-2 text-xs text-muted-foreground hover:text-foreground hover:bg-muted dark:hover:bg-white/[0.05] hover:border-foreground/20 dark:hover:border-white/[0.14] transition-colors"
            >
              ...
              Download example script template (.docx)
            </a>
          </div>
        )}
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected
                            ...
                            : "border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] hover:border-foreground/20 dark:hover:border-white/[0.12] hover:bg-muted dark:hover:bg-white/[0.04]",
                        )}
                      >
                        ...
                        {agent.keyPhrases?.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {agent.keyPhrases.slice(0, 3).map((p) => (
                              <span key={p} className="rounded px-1.5 py-0.5 text-[9px] bg-muted dark:bg-white/[0.04] text-muted-foreground border border-border dark:border-white/[0.06] italic">
                                "{p}"
                              </span>
                            ))}
                          </div>
                        ...
                        {agent.expertise?.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {agent.expertise.slice(0, 3).map((tag) => (
                              <span key={tag} className="rounded px-1.5 py-0.5 text-[9px] bg-muted dark:bg-white/[0.05] text-muted-foreground border border-border dark:border-white/[0.06]">
                                {tag}
                              </span>
                            ))}
                          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {scanResult.campaigns.map((campaign) => {
                    ...
                    const typeColor = CAMPAIGN_TYPE_COLORS[campaign.type.toLowerCase()] ?? "bg-muted dark:bg-white/[0.05] text-muted-foreground border-border dark:border-white/[0.08]";
                    return (
                      <button
                        key={campaign.name}
                          ...
                          <div className="flex flex-wrap gap-1">
                            {campaign.keyStages.slice(0, 5).map((stage, i) => (
                              <span key={stage} className="flex items-center gap-0.5 text-[9px] text-muted-foreground">
                                ...
                                <span className="rounded px-1 py-0 bg-muted dark:bg-white/[0.05] border border-border dark:border-white/[0.06]">{stage}</span>
                              </span>
                            ))}
        ...
        {phase === "preview" && flowResult && (
          <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
            ...
            {(flowResult.suggestedAgentName || flowResult.suggestedCompanyName || flowResult.suggestedBeginMessage) && (
              <div className="rounded-lg border border-border dark:border-white/[0.08] bg-muted/40 dark:bg-white/[0.02] p-3 space-y-2.5">
