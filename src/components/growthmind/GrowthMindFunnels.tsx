...
function SnapshotRow({ snap, onDelete }: { snap: any; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/40 overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-3 pt-1 border-t border-border/60 flex flex-wrap gap-2">
          {(snap.stages as FunnelStage[]).map(s => (
            <div key={s.key} className="flex items-center gap-1.5 rounded-md bg-muted/60 px-2.5 py-1.5">
