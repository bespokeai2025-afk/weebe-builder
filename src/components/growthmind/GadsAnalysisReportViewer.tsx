...
function Stat({ label, value, tone }: { label: string; value: any; tone?: string }) {
  return (
    <div className="rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] px-2.5 py-2">
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border dark:border-white/[0.06]">
      <table className="w-full text-[11px]">
        <thead>
          <tr className="bg-muted/60 dark:bg-white/[0.03] text-left">
            {columns.map(c => (
        ...
        <tbody>
          {rows.slice(0, maxRows).map((r, i) => (
            <tr key={i} className="border-t border-border/60 dark:border-white/[0.04]">
              {columns.map(c => (
      ...
      {rows.length > maxRows && (
        <p className="text-[10px] text-muted-foreground px-2.5 py-1.5 border-t border-border/60 dark:border-white/[0.04]">
...
const CLASS_TONES: Record<string, string> = {
  ...
  low_data: "bg-muted dark:bg-white/[0.06] text-muted-foreground",
};

function ClassBadge({ v }: { v: string }) {
  return (
    <span className={cn("inline-block rounded-full px-1.5 py-0.5 text-[10px] font-medium whitespace-nowrap", CLASS_TONES[v] ?? "bg-muted dark:bg-white/[0.06] text-muted-foreground")}>
        ...
        <div>
          ...
          <div className="space-y-2">
            {es.topPriorities.map((p: any, i: number) => (
              <div key={i} className="rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] px-3 py-2">
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            {s.tracking.findings.map((f: string, i: number) => (
              <li key={i} className="flex items-start gap-2 rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] px-3 py-2 text-[11px] text-foreground/85">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={() => onCsv("keywords", k.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.08] px-2 py-1 text-muted-foreground hover:text-foreground">
          ...
          {["all", ...Object.keys(st.counts ?? {})].map(c => (
            <button key={c} onClick={() => setFilter(c)}
              className={cn("text-[10px] rounded-full px-2 py-1 border",
                filter === c ? "border-violet-500/40 bg-violet-500/15 text-violet-300" : "border-border dark:border-white/[0.08] text-muted-foreground hover:text-foreground")}>
              {c.replace(/_/g, " ")}{c !== "all" ? ` (${st.counts[c]})` : ` (${st.totalUniqueTerms ?? (st.rows ?? []).length})`}
            </button>
          ))}
        </div>
        <button onClick={() => onCsv("search-terms", st.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.08] px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      <DataTable
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => onCsv("negative-candidates", n.deterministicCandidates ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.08] px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      <DataTable
      ...
      {(ads.rows ?? []).map((a: any, i: number) => (
        <div key={i} className="rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] p-3">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <span className="text-[10px] rounded-full px-1.5 py-0.5 bg-muted dark:bg-white/[0.06] text-muted-foreground">{a.type}</span>
            <span className="text-[10px] rounded-full px-1.5 py-0.5 bg-muted dark:bg-white/[0.06] text-muted-foreground">Strength: {a.adStrength ?? "—"}</span>
...
function CompetitorsTab({ s }: { s: Record<string, any> }) {
  ...
  const TONE: Record<string, string> = {
    ...
    unavailable: "bg-muted dark:bg-white/[0.06] text-muted-foreground",
  };
  return (
    <div className="space-y-3">
      <SectionError error={c.error} />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <button onClick={() => onCsv("change-requests", cr.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.08] px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      {groups.map(([g, rows]) => (
...
export function GadsAnalysisReportViewer({ report }: { report: Report }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-white/[0.07] bg-muted/25 dark:bg-white/[0.015] print:border-0" data-gads-report>
      <div className="px-4 pt-4 pb-3 border-b border-border dark:border-white/[0.06] flex items-start gap-3 flex-wrap">
        ...
        <button
          ...
          className="text-[11px] flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.08] px-2.5 py-1.5 text-muted-foreground hover:text-foreground print:hidden"
        >
          ...
          <button key={t.key} onClick={() => setTab(t.key)}
            className={cn("flex items-center gap-1 text-[11px] rounded-lg px-2.5 py-1.5 border transition-all",
              tab === t.key
                ...
                : "border-border dark:border-white/[0.07] text-muted-foreground hover:text-foreground hover:bg-muted/60 dark:hover:bg-white/[0.03]")}>
