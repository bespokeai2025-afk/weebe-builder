...
function DataTable({ columns, rows, maxRows = 200 }: {
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border">
      ...
      {rows.length > maxRows && (
        <p className="text-[10px] text-muted-foreground px-2.5 py-1.5 border-t border-border/60">
...
const CLASS_TONES: Record<string, string> = {
  ...
  low_data: "bg-muted text-muted-foreground",
};

function ClassBadge({ v }: { v: string }) {
  return (
    <span className={cn("inline-block rounded-full px-1.5 py-0.5 text-[10px] font-medium whitespace-nowrap", CLASS_TONES[v] ?? "bg-muted text-muted-foreground")}>
        ...
        <div>
          ...
          <ul className="space-y-1.5">
            {s.tracking.findings.map((f: string, i: number) => (
              <li key={i} className="flex items-start gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2 text-[11px] text-foreground/85">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={() => onCsv("keywords", k.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-muted-foreground hover:text-foreground">
          ...
          {["all", ...Object.keys(st.counts ?? {})].map(c => (
            <button key={c} onClick={() => setFilter(c)}
              className={cn("text-[10px] rounded-full px-2 py-1 border",
                filter === c ? "border-violet-500/40 bg-violet-500/15 text-violet-300" : "border-border text-muted-foreground hover:text-foreground")}>
              {c.replace(/_/g, " ")}{c !== "all" ? ` (${st.counts[c]})` : ` (${st.totalUniqueTerms ?? (st.rows ?? []).length})`}
            </button>
          ))}
        </div>
        <button onClick={() => onCsv("search-terms", st.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      <DataTable
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => onCsv("negative-candidates", n.deterministicCandidates ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      <DataTable
      ...
      {(ads.rows ?? []).map((a: any, i: number) => (
        <div key={i} className="rounded-lg border border-border bg-muted/40 p-3">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <span className="text-[10px] rounded-full px-1.5 py-0.5 bg-muted text-muted-foreground">{a.type}</span>
            <span className="text-[10px] rounded-full px-1.5 py-0.5 bg-muted text-muted-foreground">Strength: {a.adStrength ?? "—"}</span>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <button onClick={() => onCsv("change-requests", cr.rows ?? [])} className="text-[11px] flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-muted-foreground hover:text-foreground">
          <FileDown className="h-3 w-3" /> CSV
        </button>
      </div>
      {groups.map(([g, rows]) => (
...
export function GadsAnalysisReportViewer({ report }: { report: Report }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/25 print:border-0" data-gads-report>
      <div className="px-4 pt-4 pb-3 border-b border-border flex items-start gap-3 flex-wrap">
        ...
        <button
          ...
          className="text-[11px] flex items-center gap-1 rounded-lg border border-border px-2.5 py-1.5 text-muted-foreground hover:text-foreground print:hidden"
        >
