...
export function VideoPipelinePanel({ campaignId }: { campaignId?: string | null }) {
  ...
  const selectBtn = (active: boolean) => cn(
    ...
    active ? "border-violet-500/40 bg-violet-500/15 text-violet-200" : "border-border text-muted-foreground/60 hover:text-foreground",
  );
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {(jobs as VideoPipelineJob[]).map(job => (
          <div key={job.id} className="rounded-xl border border-border bg-card/60 p-4 space-y-3">
