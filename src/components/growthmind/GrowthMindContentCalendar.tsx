...
function EntryModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-xl border border-border dark:border-white/[0.08] bg-card shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-end gap-2 px-5 py-3.5 border-t border-border dark:border-white/[0.06]">
        ...
        {days.map((day, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                "border-b border-r border-border/60 dark:border-white/[0.04] min-h-[96px] p-1 group",
...
function WeekView({ pivot, entries, onAddDay, onEdit }: { pivot: Date; entries: CalendarEntry[]; onAddDay: (d: Date) => void; onEdit: (e: CalendarEntry) => void }) {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-white/[0.06] overflow-hidden">
      <div className="grid grid-cols-7 border-b border-border dark:border-white/[0.06]">
        {days.map((d, i) => (
          <div key={i} className={cn("text-center py-2 border-r border-border/60 dark:border-white/[0.04] last:border-r-0", isSameDay(d, today) && "bg-emerald-500/5")}>
      ...
      <div className="grid grid-cols-7 min-h-[200px]">
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className={cn("border-r border-border/60 dark:border-white/[0.04] last:border-r-0 p-2 space-y-1 group", isSameDay(d, today) && "bg-emerald-500/[0.03]")}>
        ...
        <div key={week}>
          ...
          <div className="space-y-1">
            {items.map(e => {
              ...
              return (
                <div key={e.id} className="flex items-center gap-3 rounded-lg border border-border dark:border-white/[0.06] bg-card/40 px-3 py-2 group hover:bg-card/70 transition-colors">
...
function CampaignPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-border dark:border-white/[0.08] bg-card shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border dark:border-white/[0.06]">
        ...
        <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            {campaigns.map(c => (
              <div key={c.id} className="flex items-center gap-2 rounded-lg border border-border dark:border-white/[0.06] bg-card/40 px-3 py-2">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-border dark:border-white/[0.08] bg-card shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            {seriesList.map(s => (
              <div key={s.id} className="flex items-center gap-2 rounded-lg border border-border dark:border-white/[0.06] bg-card/40 px-3 py-2">
    ...
    <GrowthMindShell>
      <div className="px-6 py-5 max-w-6xl">
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div className="flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.06] bg-card/40 p-0.5">
            {(["month", "week", "day", "quarter", "90day"] as CalView[]).map(v => (
              <button
                ...
                className={cn(
                  ...
                  view === v ? "bg-emerald-600 text-white" : "text-muted-foreground hover:text-foreground hover:bg-muted dark:hover:bg-white/[0.04]",
                )}
              >
                {VIEW_LABELS[v]}
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Nav */}
            <div className="flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.06] bg-card/40">
        ...
        {isLoading ? (
          ...
          <>
            ...
            {view === "day" && (
              <div className="rounded-xl border border-border dark:border-white/[0.06] p-6 min-h-[300px]">
                ...
                <div className="space-y-2">
                  {entries.filter(e => e.scheduledDate && isSameDay(new Date(e.scheduledDate), pivot)).map(e => (
                    <div key={e.id} className="flex items-center gap-3 p-3 rounded-lg border border-border dark:border-white/[0.06] bg-card/40 hover:bg-card/70 cursor-pointer" onClick={() => openEdit(e)}>
