...
function renderMarkdown(md: string): string {
  ...
  return safe
    ...
    .replace(/\*(.+?)\*/g, "<em>$1</em>")
    .replace(/`(.+?)`/g, '<code class="rounded bg-muted px-1 py-0.5 text-[11px] text-sky-300">$1</code>')
...
function PublishSettingsModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-background/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl border border-border bg-[hsl(var(--sidebar-background))] shadow-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex-1 min-h-0 flex gap-0 overflow-hidden">
            ...
            <div className="flex-1 min-w-0 overflow-y-auto p-6 space-y-4">
              ...
              <details className="rounded-xl border border-border overflow-hidden">
                <summary className="px-4 py-2.5 text-xs font-semibold cursor-pointer select-none hover:bg-muted/40 flex items-center gap-2">
                  <Eye className="h-3.5 w-3.5 text-muted-foreground" /> Preview Rendered Output
                </summary>
                <div
                  className="px-5 py-4 prose prose-invert prose-sm max-w-none border-t border-border/60"
  ...
  return (
    <GrowthMindShell>
      <div className="flex flex-col h-full min-h-0 overflow-y-auto">
        {/* Header */}
        <div className="border-b border-border px-6 py-5 flex items-center gap-3 shrink-0">
