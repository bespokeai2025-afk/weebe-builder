      ...
      {visible.map((s, i) => {
        ...
        return (
          <div key={s.id} className="flex items-center gap-1.5">
            ...
            {i < visible.length - 1 && <div className="h-px w-3 bg-muted" />}
          </div>
        );
      })}
    </div>
          ...
          {clips.map(clip => (
            <div
              ...
              className={cn(
                ...
                : "bg-muted/60 border-border text-muted-foreground/40",
              )}
            >
...
function VideoAssetCard({ asset, onDelete, onSchedule, onRetry }: {
  ...
  return (
    <div className="rounded-xl border border-border bg-card/60 p-4 space-y-3 hover:border-foreground/20 transition-all">
      ...
      {showDebug && (() => {
        ...
        return (
          <div className="rounded-lg border border-border/60 bg-muted/40 p-2.5 space-y-1 font-mono">
            {rows.map(([k, v]) => (
...
function ScheduleModal({ asset, onClose, onScheduled }: {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-background/70 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border bg-[hsl(var(--sidebar-background))] shadow-2xl p-6 space-y-5">
      ...
      <div className="flex h-full min-h-0">
        ...
        <div className="flex-1 min-w-0 overflow-y-auto p-6 space-y-6">
          ...
          <div className="rounded-2xl border border-border bg-card/60 p-5 space-y-5">
            ...
            {/* Mode tabs */}
            <div className="flex rounded-xl border border-border overflow-hidden p-0.5 bg-muted/40">
              <button
                    ...
                    {isVeo2 && (
                      <div className="flex items-center gap-1.5 rounded-lg border border-red-500/20 bg-red-500/[0.05] px-2.5 py-1.5">
                        ...
                        <p className="text-[10px] text-red-400/90">
                          ...
                          clear the "Veo Model" field (or set it to <code className="bg-muted rounded px-1">veo-3.0-generate-preview</code>).
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            disabled ? "opacity-35 cursor-not-allowed border-border/60"
                            : active
                              ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-300"
                              : "border-border text-muted-foreground/60 hover:text-foreground hover:border-foreground/20",
                          )}
                        >
                ...
                <div className="rounded-xl border border-violet-500/15 bg-violet-500/[0.04] p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {([
                      ...
                      <div key={d.label} className="rounded-lg bg-muted/60 border border-border p-2 text-center">
                        <p className={cn(
                          "text-sm font-bold tabular-nums",
        ...
        <aside className="hidden xl:flex w-64 shrink-0 flex-col border-l border-border p-4 gap-4 overflow-y-auto">
          <VideoCostPanel />
          ...
          <div className="rounded-xl border border-border bg-card/60 p-4 space-y-2.5">
            ...
            <div className="space-y-2">
              ...
              <div className="border-t border-border pt-2 text-[10px] text-muted-foreground/50 space-y-0.5">
