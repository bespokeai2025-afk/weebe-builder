    ...
    <div className="border-b border-border bg-[hsl(var(--card))] px-5 py-4 space-y-4">
        ...
        <div className="flex gap-1.5">
          {SPEED_OPTIONS.map(s => (
            <button key={s} onClick={() => onChange({ ...settings, speed: s })} className={cn(
              ...
              settings.speed === s ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-300" : "border-border text-muted-foreground hover:text-foreground",
            )}>{s}×</button>
          ))}
        </div>
      </div>
        ...
        <div className="flex gap-1.5">
          {PERSONALITIES.map(p => (
            <button key={p} onClick={() => onChange({ ...settings, personality: p })} className={cn(
              ...
              settings.personality === p ? "border-emerald-500/40 bg-emerald-500/15 text-emerald-300" : "border-border text-muted-foreground hover:text-foreground",
            )}>{p}</button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-1 border-t border-border">
        ...
        <button
          ...
          className={cn("w-9 h-5 rounded-full relative transition-all", settings.autoPlay ? "bg-emerald-500" : "bg-muted")}
        >
  ...
  return (
    <div className={cn("flex gap-2.5 max-w-[85%]", isGM ? "self-start" : "self-end flex-row-reverse")}>
      <div className={cn(
        ...
        isGM ? "bg-emerald-500/20 ring-1 ring-emerald-500/30" : "bg-muted",
      )}>
      ...
      <div className={cn("flex flex-col gap-0.5", isGM ? "items-start" : "items-end")}>
        <div className={cn(
          ...
          isGM ? "bg-emerald-500/[0.08] border border-emerald-500/15" : "bg-muted border border-border",
        )}>
          {isEmpty
        ...
        <div className="border-b border-border px-5 py-3 flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            className={cn(
              ...
              settingsOpen ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-300" : "border-border text-muted-foreground hover:text-foreground",
            )}
          >
              ...
              <div className="flex flex-col gap-1.5">
                {SUGGESTED.map(q => (
                  <button
                    key={q}
                    onClick={() => send(q)}
                    className="text-left text-xs px-3 py-1.5 rounded-lg border border-border bg-muted/40 hover:bg-emerald-500/[0.08] hover:border-emerald-500/20 hover:text-foreground text-muted-foreground transition-all"
                  >
                    {q}
                  </button>
                ))}
        ...
        <div className="border-t border-border px-4 py-3 shrink-0">
          <div className="flex items-end gap-2">
            <textarea
              ...
              rows={1}
              className="flex-1 resize-none rounded-xl border border-border bg-muted/60 px-3.5 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-emerald-500/30 transition-colors min-h-[42px] max-h-32"
              style={{ height: "auto" }}
              onInput={e => {
