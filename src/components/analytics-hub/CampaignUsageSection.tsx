        ...
        {rows.length === 0 ? (
          ...
          <div className="max-h-96 overflow-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((c: any) => (
                  <tr key={c.campaignId ?? "unassigned"} className="h-10 border-b border-border/60 hover:bg-muted/40">
                    <td className="px-3 py-2 font-medium">
                      {c.campaignName}
                      {c.campaignId == null && (
                        <span className="ml-2 rounded bg-muted px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">unassigned</span>
                      )}
                    </td>
