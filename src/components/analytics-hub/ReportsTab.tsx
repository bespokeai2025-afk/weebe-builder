        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="rounded-lg border border-border bg-card/60 py-1.5 pl-8 pr-3 text-sm"
            />
          </div>
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="rounded-lg border border-border bg-card/60 px-2.5 py-1.5 text-sm text-foreground"
          >
        ...
        {reportsQ.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {reports.map((r) => (
                  <tr key={r.id} className="h-11 border-b border-border/60 hover:bg-muted/40">
        ...
        {schedQ.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {schedules.map((s) => (
                  <tr key={s.id} className="h-11 border-b border-border/60 hover:bg-muted/40">
...
function PrefToggle({ on, disabled, onClick }: { on: boolean; disabled: boolean; onClick: () => void }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`relative h-5 w-9 shrink-0 rounded-full transition-colors disabled:opacity-50 ${on ? "bg-primary" : "bg-muted"}`}
      aria-pressed={on}
    >
  ...
  return (
    <div className="space-y-1.5">
      <input
        ...
        disabled={disabled}
        className="w-full rounded-lg border border-border bg-card/60 px-2.5 py-1.5 text-xs disabled:opacity-50"
      />
      {dirty && !disabled && (
    ...
    <div className="rounded-xl border border-border bg-card/40 p-3.5">
      ...
      <div className="mb-2 flex items-center gap-2">
        ...
        <select
          ...
          className="rounded-lg border border-border bg-card/60 px-2 py-1 text-xs capitalize disabled:opacity-50"
        >
...
function ModalShell({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="flex max-h-[85dvh] w-full max-w-lg flex-col rounded-2xl border border-border bg-popover p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
...
function MetricsGrid({ metrics }: { metrics: any }) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 rounded-lg border border-border bg-card/40 p-3 sm:grid-cols-3">
        {entries.map(([k, v]) => (
    ...
    <ModalShell title="Generate report" onClose={onClose}>
      <div className="space-y-3">
        <label className="block text-sm">
          ...
          <select value={reportType} onChange={(e) => setReportType(e.target.value)} className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm">
            {reportTypes.map((t) => <option key={t.key} value={t.key}>{t.label}</option>)}
          </select>
        </label>
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-muted-foreground">Name (optional)</span>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Auto-generated if empty" className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm" />
        </label>
    ...
    <ModalShell title="New report schedule" onClose={onClose}>
      <div className="space-y-3">
        <label className="block text-sm">
          ...
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Weekly workspace summary" className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm" />
        </label>
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-muted-foreground">Report type</span>
          <select value={reportType} onChange={(e) => setReportType(e.target.value)} className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm">
        ...
        <label className="block text-sm">
          ...
          <select value={frequency} onChange={(e) => setFrequency(e.target.value as any)} className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm capitalize">
            {FREQUENCIES.map((f) => <option key={f} value={f}>{f}</option>)}
          </select>
        </label>
        <label className="block text-sm">
          <span className="mb-1 block text-xs font-medium text-muted-foreground">Recipients (comma-separated emails)</span>
          <input value={recipients} onChange={(e) => setRecipients(e.target.value)} placeholder="ops@example.com, cfo@example.com" className="w-full rounded-lg border border-border bg-card/60 px-3 py-2 text-sm" />
        </label>
        <div className="flex justify-end gap-2 pt-1">
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button
