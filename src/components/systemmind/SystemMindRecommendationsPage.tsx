      ...
      <div className="mx-auto max-w-4xl px-4 py-6 md:px-8">
          ...
          {(["open", "dismissed", "all"] as const).map((f) => (
            <button key={f} onClick={() => setStatusFilter(f)}
              className={cn(
                ...
                statusFilter === f ? "bg-sky-500/20 text-sky-300" : "bg-muted dark:bg-white/[0.04] text-muted-foreground hover:bg-muted dark:hover:bg-white/[0.08]",
              )}>
            ...
            <button key={cat} onClick={() => setCategoryFilter(cat)}
              className={cn(
                ...
                categoryFilter === cat
                  ...
                  : "bg-muted/60 dark:bg-white/[0.03] text-muted-foreground/70 hover:bg-muted dark:hover:bg-white/[0.07] border border-border dark:border-white/[0.06]",
              )}>
              {cat}
            </button>
          ))}
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {sorted.map((rec: any) => (
              <div key={rec.id} className={cn("rounded-xl border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] p-4", rec.dismissed_at && "opacity-50")}>
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                      <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide", PRIORITY_BADGE[rec.priority] ?? "bg-muted dark:bg-white/[0.05] text-muted-foreground")}>
                        {rec.priority}
                      </span>
                  ...
                  {!rec.dismissed_at && (
                    <button
                      ...
                      className="shrink-0 rounded p-1 text-muted-foreground hover:text-foreground hover:bg-muted dark:hover:bg-white/[0.06] transition-colors"
                      title="Dismiss"
                    >
