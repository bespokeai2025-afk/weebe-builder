...
function CheckRow({ c }: { c: { key: string; label: string; status: string; detail: string } }) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-border bg-muted/40 px-2.5 py-2">
            ...
            {(candidates ?? []).map((c: any) => (
              <button
                ...
                className={cn(
                  ...
                  selectedCall === c.id ? "border-sky-500/40 bg-sky-500/[0.08]" : "border-border bg-muted/40 hover:bg-muted",
                )}
              >
      ...
      {(state.history ?? []).length > 0 && (
        <div className="space-y-1.5">
          ...
          {showHistory && (state.history as any[]).map((h) => (
            <div key={h.id} className="flex items-center gap-2 rounded-md border border-border/60 bg-muted/20 px-2.5 py-1.5">
