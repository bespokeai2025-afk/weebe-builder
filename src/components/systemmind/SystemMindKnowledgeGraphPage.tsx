        ...
        <>
          ...
          <div className="space-y-1 max-h-72 overflow-y-auto">
            {q.data.edges.map((e) => {
              ...
              return (
                <div key={e.id} className="flex items-center gap-1.5 text-[10px] rounded-md bg-muted/40 px-2 py-1.5">
            ...
            {presentTypes.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  nodeType === t ? "border-indigo-500/40 bg-indigo-500/[0.1]" : "border-border hover:bg-muted",
                )}
              >
                ...
                {(nodes.data ?? []).map((n) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedNode === n.id ? "border-indigo-500/40 bg-indigo-500/[0.08]" : "border-border bg-muted/40 hover:bg-muted",
                    )}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                      ...
                      {(n.tags ?? []).slice(0, 3).map((t) => (
                        <span key={t} className="text-[9px] border border-border rounded px-1 py-0.5 text-muted-foreground/60">{t}</span>
                      ))}
                    </div>
                  </button>
                ))}
