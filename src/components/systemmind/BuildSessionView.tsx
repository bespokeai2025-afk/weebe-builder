...
function BuildProgress({ phases }: { phases: string[] }) {
  ...
  return (
    <div className="max-w-[85%] space-y-1.5 rounded-lg border border-border bg-muted/60 px-3 py-2.5">
    ...
    <div className="space-y-3">
      <Section icon={WorkflowIcon} title={`Workflow — ${wf.name ?? "Unnamed"}`}>
        ...
        <div className="space-y-1">
          {steps.map((s: any) => (
            <div key={s.id} className="flex items-start gap-2 rounded border border-border bg-muted/40 px-2 py-1.5">
          ...
          <div className="flex flex-wrap items-center gap-1">
            {p.steps.map((s: any, j: number) => (
              <span key={j} className="inline-flex items-center gap-1">
                <span className="rounded bg-muted border border-border px-1.5 py-0.5 text-[10px]" title={s.description}>
                  {s.type}
                </span>
...
function VariablesPanel({ config }: { config: Record<string, any> | null }) {
  ...
  const row = (v: any, kind: "variable" | "extracted", i: number) => (
    <div key={`${kind}${i}`} className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
  ...
  return (
    <div className="space-y-2 rounded-lg border border-sky-500/25 bg-sky-500/[0.04] p-3">
      ...
      <p className="text-[10px] text-muted-foreground">
        ...
        <code className="rounded bg-muted px-1">{"{{variable}}"}</code> in the script, filled
      ...
      {unmapped.length > 0 && (
        <div className="space-y-2">
          ...
          {unmapped.map((f) => {
            const n = nameOf(f);
            return (
              <div key={n} className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-2.5 py-2">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-md border border-border bg-muted/40 px-2.5 py-2">
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3">
      ...
      <div className="flex min-h-0 flex-1 gap-3">
        {/* Chat column */}
        <div className={cn(
          "flex min-h-0 flex-col rounded-xl border border-border bg-muted/20",
                ...
                <div className="space-y-1.5">
                  {STARTER_PROMPTS.map((p) => (
                    <button
                      key={p}
                      onClick={() => setPrompt(p)}
                      className="w-full rounded-lg border border-border bg-muted/40 px-3 py-2 text-left text-[11px] leading-relaxed text-muted-foreground transition-colors hover:border-sky-500/30 hover:bg-sky-500/[0.06] hover:text-foreground"
                    >
            ...
            {messages.map((m: any) => (
              <div key={m.id} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  m.role === "systemmind" && "border border-border bg-muted/60",
              ...
              <div className="mb-2 flex flex-wrap gap-1.5">
                {ITERATION_PROMPTS.map((p) => (
                  <button
                    key={p}
                    onClick={() => setPrompt(p)}
                    className="rounded-full border border-border bg-muted/40 px-2.5 py-1 text-[10px] text-muted-foreground transition-colors hover:border-sky-500/30 hover:bg-sky-500/[0.06] hover:text-foreground"
                  >
                    {p}
                  </button>
                ))}
            ...
            {stepTabs.map(([key, Icon, label], i) => (
              <button
                ...
                className={cn(
                  ...
                  tab === key ? "bg-sky-500/15 text-sky-300" : "text-muted-foreground hover:bg-muted",
                )}
              >
                <span className={cn(
                  "hidden h-3.5 w-3.5 items-center justify-center rounded-full text-[8px] font-bold xl:flex",
                  tab === key ? "bg-sky-500/30 text-sky-200" : "bg-muted text-muted-foreground",
                )}>
                  {i + 1}
                </span>
              ...
              <div className="space-y-2">
                ...
                {snapshots.length > 0 && (
                  <div className="space-y-2 pt-3">
                    ...
                    {snapshots.map((s: any) => (
                      <div key={s.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-muted/40 px-3 py-2">
        ...
        <DialogContent className="max-h-[85dvh] max-w-lg overflow-y-auto">
          ...
          {safetyReport && (() => {
            ...
            return (
              <div className="space-y-3">
                {/* Target */}
                <div className="rounded-lg border border-border bg-muted/40 p-3 text-[11px]">
                  {impact.targetIsNew ? (
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            applyMode === value ? "border-sky-500/40 bg-sky-500/[0.08]" : "border-border bg-muted/40 hover:bg-muted",
