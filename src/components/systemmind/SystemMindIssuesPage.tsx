    ...
    <div className={cn("rounded-xl border p-4 flex gap-3", SEVERITY_STYLE[issue.severity])}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-1.5 mb-1">
          ...
          <span className={cn("rounded px-1.5 py-0.5 text-[10px] font-medium capitalize", CATEGORY_BADGE[issue.category] ?? "bg-muted text-muted-foreground")}>
            {issue.category}
          </span>
        </div>
          ...
          {(["all", "critical", "high", "medium", "low"] as const).map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={cn(
                ...
                filter === s ? "bg-sky-500/20 text-sky-300" : "bg-muted text-muted-foreground hover:bg-muted",
              )}>
