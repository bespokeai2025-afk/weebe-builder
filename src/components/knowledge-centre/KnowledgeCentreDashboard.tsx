...
function platformAccent(slug: string): string {
  return KB_ACCENT[slug.replace("platform_", "")] ?? "text-muted-foreground bg-muted/30 dark:bg-white/[0.04] ring-border dark:ring-white/[0.08]";
}
                  ...
                  {mindKeys.map((mind) => {
                    ...
                    return (
                      <div key={mind}>
                        ...
                        <div className="h-1 rounded-full bg-muted/40 dark:bg-white/[0.06] overflow-hidden">
                          <div
              ...
              {DEFAULT_EXECUTIVE_KBS.map((kb) => {
                ...
                return (
                  <Link
                    ...
                    params={{ slug: kb.slug }}
                    className="group rounded-xl border border-border dark:border-white/[0.06] bg-muted/20 dark:bg-white/[0.02] p-5 transition-colors hover:border-border dark:hover:border-white/[0.12] hover:bg-muted/30 dark:hover:bg-white/[0.04]"
                  >
              ...
              <div className="space-y-1.5">
                {data!.recentQueries.map((q: any) => (
                  <div key={q.id} className="flex items-center gap-3 text-[11px]">
                    <span className={cn("shrink-0 rounded px-1.5 py-0.5 font-medium capitalize", MIND_COLOR[q.mind_type] ?? "text-foreground", "bg-muted/30 dark:bg-white/[0.04]")}>
