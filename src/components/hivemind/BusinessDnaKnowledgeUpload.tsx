    ...
    <div className="rounded-2xl border border-border bg-[hsl(var(--card))] overflow-hidden">
      ...
      <div className="px-5 pb-5 space-y-4">
        ...
        <button
          ...
          disabled={uploading}
          className="flex w-full flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed border-border bg-muted/40 py-6 transition-colors hover:border-foreground/25 hover:bg-muted disabled:opacity-60"
        >
        ...
        {isLoading ? (
          ...
          <p className="rounded-xl border border-border bg-muted/40 py-6 text-center text-xs text-muted-foreground">
            No documents yet. Upload reference material to sharpen your Business DNA and GrowthMind's knowledge.
          </p>
        ) : (
          <div className="space-y-1.5">
            {docs!.map((d: any) => (
              <div key={d.id} className="flex items-center gap-2.5 rounded-lg border border-border bg-muted/40 px-3 py-2.5">
                ...
                <button onClick={() => handleReindex(d.id)} title="Re-index" className="rounded-md p-1 text-muted-foreground hover:bg-muted hover:text-foreground">
