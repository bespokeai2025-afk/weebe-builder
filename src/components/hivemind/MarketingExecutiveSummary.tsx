...
const SEVERITY_STYLE: Record<string, string> = {
  ...
  low:      "border-border dark:border-white/[0.08] bg-muted/60 dark:bg-white/[0.03] text-muted-foreground",
};
...
export function MarketingExecutiveSummary({
  ...
  if (selfFetch && isLoading) {
    return (
      <div className={cn("rounded-xl border border-border dark:border-white/[0.07] bg-[hsl(var(--card))] px-4 py-6 flex items-center justify-center gap-2 text-muted-foreground", className)}>
  ...
  if (!gm) {
    return (
      <div className={cn("rounded-xl border border-border dark:border-white/[0.07] bg-[hsl(var(--card))] px-4 py-5 text-center", className)}>
  ...
  return (
    <div className={cn("rounded-xl border border-violet-500/20 bg-gradient-to-br from-violet-500/[0.05] to-transparent overflow-hidden", className)}>
      {/* Header */}
      <div className="px-4 py-3 border-b border-border dark:border-white/[0.06] flex items-center gap-3">
          ...
          <Section icon={Lightbulb} iconClass="text-violet-400" label="CMO Recommendations">
            <div className="space-y-2">
              {gm.recommendedActions.slice(0, 4).map((a) => (
                <div key={a.id} className="rounded-lg border border-border dark:border-white/[0.07] bg-muted/40 dark:bg-white/[0.02] px-3 py-2.5">
        ...
        {gm.missingMarketingAssets.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {gm.missingMarketingAssets.map((m, i) => (
              <span key={i} className="text-[10px] rounded border border-border dark:border-white/[0.08] bg-muted/60 dark:bg-white/[0.03] px-1.5 py-0.5 text-muted-foreground">{m}</span>
            ))}
          </div>
        )}
      </div>
