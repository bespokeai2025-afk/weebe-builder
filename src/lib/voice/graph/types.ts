...
export interface FlowTransitionCondition {
  ...
  prompt: string;
  /** Retell join: || = if any, && = if all. */
  operator?: "||" | "&&";
  /** Structured Retell clauses. Preferred over parsing `prompt`. */
...
export interface ToolInvocation {
  toolId: string;
  toolName: string;
  /** `local`, `webhook`, `end_call`, `mcp` … as exported by the builder. */
  toolType: string;
  url?: string;
  timeoutMs?: number;
  method?: string;
