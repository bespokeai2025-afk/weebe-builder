...
function VoiceSettingsPanel({ settings, onChange, onClose, voices }: {
  ...
  return (
    <div className="border-b border-border bg-[hsl(var(--card))] px-5 py-4 space-y-4">
        ...
        <div className="flex gap-1.5">
          {SPEED_OPTIONS.map(s => (
            <button key={s} onClick={() => onChange({ ...settings, speed: s })} className={cn(
              ...
              settings.speed === s ? "border-violet-500/40 bg-violet-500/15 text-violet-300" : "border-border text-muted-foreground hover:text-foreground",
            )}>{s}×</button>
          ))}
        </div>
      </div>
        ...
        <div className="flex gap-1.5">
          {PERSONALITIES.map(p => (
            <button key={p} onClick={() => onChange({ ...settings, personality: p })} className={cn(
              ...
              settings.personality === p ? "border-violet-500/40 bg-violet-500/15 text-violet-300" : "border-border text-muted-foreground hover:text-foreground",
            )}>{p}</button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-1 border-t border-border">
        ...
        <button
          ...
          className={cn("w-9 h-5 rounded-full relative transition-all", settings.autoPlay ? "bg-violet-500" : "bg-muted")}
        >
...
function HintPanel({ onSelect, onClose }: { onSelect: (q: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="border-t border-border bg-[hsl(var(--background))] flex flex-col max-h-72">
      {/* Header */}
      ...
      {/* Category tabs */}
      <div className="flex overflow-x-auto gap-1 px-3 py-1.5 border-b border-border/60 shrink-0 scrollbar-none" style={{ scrollbarWidth: "none" }}>
        {HINT_GROUPS.map((g, i) => (
          <button
            key={g.label}
      ...
      <div className="overflow-y-auto px-3 py-2 flex flex-col gap-1.5">
        {group.questions.map(({ q, desc }) => (
          <button
            ...
            className="text-left rounded-lg border border-border bg-muted/40 hover:bg-violet-500/[0.08] hover:border-violet-500/20 px-3 py-2 transition-all group"
          >
...
function WorkOrderProposalCard({ wo }: { wo: WorkOrderProposal }) {
  ...
  return (
    <div className="mt-2 w-full rounded-xl border border-violet-500/25 bg-violet-500/[0.06] px-3.5 py-3 space-y-2.5">
      ...
      {linkedAction && (
        <div className="rounded-lg border border-border bg-muted/60 px-2.5 py-2 space-y-1.5">
  ...
  return (
    <div className={cn("flex gap-2.5 max-w-[85%]", isHive ? "self-start" : "self-end flex-row-reverse")}>
      <div className={cn(
        ...
        isHive ? "bg-violet-500/20 ring-1 ring-violet-500/30" : "bg-muted",
      )}>
      ...
      <div className={cn("flex flex-col gap-0.5", isHive ? "items-start" : "items-end")}>
        <div className={cn(
          ...
          isHive ? "bg-violet-500/[0.08] border border-violet-500/15" : "bg-muted border border-border",
        )}>
          {isEmpty
    ...
    <HiveMindShell>
      ...
      <div className="sticky top-0 z-20 border-b border-border bg-[hsl(var(--background))]/95 backdrop-blur-sm px-5 py-3 flex items-center gap-3">
        ...
        <button
          ...
          className={cn("p-1.5 rounded-lg transition-colors", settingsOpen ? "bg-muted text-violet-400" : "hover:bg-muted text-muted-foreground")}
        >
      ...
      <div className="px-5 py-4 flex flex-col gap-3 pb-2">
        ...
        {mode === "live" && (
          <div className={cn(
            ...
            liveActive ? "border-red-500/20 bg-red-500/[0.04]" : "border-border bg-muted/40",
          )}>
            {liveActive ? (
              <>
          ...
          <div className="border-t border-border px-4 py-3">
            <div className="flex items-end gap-2">
              ...
              <textarea
                ...
                rows={1}
                disabled={isThinking}
                className="flex-1 resize-none rounded-xl border border-border bg-muted px-3.5 py-2 text-sm placeholder:text-muted-foreground/50 focus:outline-none focus:border-violet-500/40 focus:ring-1 focus:ring-violet-500/20 disabled:opacity-60 max-h-32 leading-relaxed"
