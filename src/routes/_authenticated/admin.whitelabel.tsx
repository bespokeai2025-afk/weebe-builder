    ...
    <div className="space-y-5">
          ...
          {([["primary_color", "Primary"], ["secondary_color", "Background"], ["accent_color", "Accent"]] as [keyof WhitelabelPartner, string][]).map(([key, label]) => (
            <div key={key} className="flex items-center gap-2">
              <input
                ...
                className="h-8 w-8 cursor-pointer rounded border border-border dark:border-white/10 bg-transparent p-0.5"
              />
            ...
            {PARTNER_TIERS.map((t) => (
              <button
                ...
                className={cn(
                  ...
                  form.partner_tier === t.id ? "border-current" : "border-border dark:border-white/[0.08] text-muted-foreground"
                )}
      ...
      <div className="flex flex-wrap gap-4">
        <label className="flex items-center gap-2 cursor-pointer">
          <div
            ...
            className={cn("h-5 w-5 rounded border flex items-center justify-center transition-colors", form.hide_powered_by ? "bg-primary border-primary" : "border-border dark:border-white/20")}
          >
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          <div
            ...
            className={cn("h-5 w-5 rounded border flex items-center justify-center transition-colors", form.active ? "bg-primary border-primary" : "border-border dark:border-white/20")}
          >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? "border-primary/30 bg-card" : "border-border dark:border-white/[0.06] hover:border-border dark:hover:border-white/15"
                )}
              >
                <div
      ...
      <div className="space-y-1.5">
        ...
        <textarea
          ...
          rows={2}
          className="w-full rounded-md border border-border dark:border-white/[0.06] bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary/40 resize-none"
      ...
      {!isLoading && (partners as any[]).length > 0 && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={stat.label} className="rounded-xl border border-border dark:border-white/[0.06] bg-card/40 p-3">
          ...
          {(partners as any[]).map((p: any) => (
            <div
              ...
              className={cn(
                ...
                p.active ? "border-border dark:border-white/[0.08] bg-card/40" : "border-border dark:border-white/[0.04] bg-card/20 opacity-60"
              )}
            >
              {/* Color stripe */}
              ...
              <div className="flex items-center gap-1.5">
                ...
                {[p.primary_color, p.secondary_color, p.accent_color].map((c, i) => (
                  <div key={i} className="h-4 w-4 rounded-full border border-border dark:border-white/10" style={{ background: c }} title={c} />
                ))}
                {p.hide_powered_by && <span className="ml-1 text-[10px] text-muted-foreground">· White labelled</span>}
              </div>

              <div className="flex gap-2 mt-auto pt-2 border-t border-border dark:border-white/[0.06]">
                <Button
                  variant="outline"
                  size="sm"
