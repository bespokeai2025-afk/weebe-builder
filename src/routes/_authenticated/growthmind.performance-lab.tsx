    ...
    <div className="space-y-6 p-6">
      ...
      <div className="rounded-xl border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] p-4">
        ...
        {proposed.length === 0 ? (
          ...
          <ul className="mt-2 space-y-2">
            {proposed.map((p: any) => (
              <li key={p.id} className="rounded-lg bg-muted/60 dark:bg-white/[0.03] px-3 py-2.5">
      ...
      <div className="space-y-3">
        ...
        {posts.map((post: any) => {
          ...
          return (
            <div key={post.id} className="rounded-xl border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
                {CATEGORY_META.map(({ key, label, icon: Icon }) => (
                  <div key={key} className="rounded-lg bg-muted/60 dark:bg-white/[0.03] px-3 py-2">
                  ...
                  return (
                    <span key={key} title={hadError ? `Captured with an issue: ${hadError}` : s ? `Captured ${new Date(s.captured_at).toLocaleString()}` : "Not yet captured"}
                      className={cn("rounded-full border px-2 py-0.5 text-[11px]",
                        ...
                        : "border-border dark:border-white/10 text-muted-foreground/60")}>
                      {key}{s && hadError ? " !" : ""}
                    </span>
                  );
                })}
