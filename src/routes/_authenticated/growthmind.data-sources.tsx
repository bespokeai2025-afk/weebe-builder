...
function SourceCard({ source }: { source: DataSource }) {
  ...
  return (
    <div className={cn("rounded-xl border border-border dark:border-white/[0.06] p-4 flex flex-col gap-3", cfg.bg)}>
      ...
      <div className="flex items-center justify-between gap-2 mt-auto">
        <div className="flex flex-wrap gap-1">
          {source.usedBy.slice(0, 2).map(u => (
            <span key={u} className="text-[10px] bg-muted dark:bg-white/[0.04] text-muted-foreground px-1.5 py-0.5 rounded">
              {u}
            </span>
          ))}
