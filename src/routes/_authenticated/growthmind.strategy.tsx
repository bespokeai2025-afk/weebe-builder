...
function StrategySection({ title, icon: Icon, content }: { title: string; icon: React.ElementType; content: string }) {
  ...
  return (
    <div className="border-b border-border/60 dark:border-white/[0.04] last:border-0">
      <button
...
function StrategyCard({ strategy, onDelete, onSend }: {
  ...
  return (
    <div className="rounded-xl border border-border dark:border-white/[0.06] bg-card/60 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-border/60 dark:border-white/[0.04] flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            {strategy.channels.slice(0, 3).map(ch => (
              <span key={ch} className="text-[10px] bg-muted dark:bg-white/[0.04] text-muted-foreground px-1.5 py-0.5 rounded">{ch}</span>
            ))}
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-5 py-2">
          ...
          {strategy.kpis.length > 0 && (
            <div className="border-b border-border/60 dark:border-white/[0.04] last:border-0 pb-3">
              ...
              <div className="space-y-1.5">
                {strategy.tasks.map((task, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs">
                    ...
                    <span className="text-[10px] bg-muted dark:bg-white/[0.04] text-muted-foreground px-1.5 py-0.5 rounded shrink-0">
                      {task.channel}
                    </span>
                  </div>
                ))}
          ...
          {strategy.campaigns.length > 0 && (
            <div className="border-t border-border/60 dark:border-white/[0.04] pb-3">
              <p className="text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground mb-2 pt-3 flex items-center gap-2">
                ...
                {strategy.campaigns.map((c, i) => (
                  <div key={i} className="rounded-lg bg-muted/60 dark:bg-white/[0.03] px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] bg-muted dark:bg-white/[0.04] text-muted-foreground px-1.5 py-0.5 rounded">{c.timeline}</span>
                    </div>
  ...
  return (
    <GrowthMindShell>
      <div className="px-6 py-5 max-w-3xl space-y-5">
        ...
        {/* Period tabs */}
        <div className="flex items-center gap-1 bg-muted/60 dark:bg-white/[0.03] rounded-lg p-1 w-fit">
          {PERIODS.map(p => (
            <button
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-dashed border-border dark:border-white/[0.12] bg-card/40 flex flex-col items-center justify-center py-16 gap-4">
