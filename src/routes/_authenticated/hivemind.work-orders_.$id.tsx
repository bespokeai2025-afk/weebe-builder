...
function StageCard({ task, index, total }: { task: WorkOrderStageTask; index: number; total: number }) {
  ...
  return (
    <div className="relative flex gap-4">
      {/* Connector line */}
      {index < total - 1 && (
        <div className="absolute left-[19px] top-10 bottom-0 w-px bg-muted dark:bg-white/[0.06]" />
      )}
      ...
      <div className={cn(
        ...
        isComplete
          ...
          : "border-border dark:border-white/[0.1] bg-muted/60 dark:bg-white/[0.03]",
      )}>
        {isComplete
      ...
      <div className={cn(
        ...
        isComplete
          ...
          : "border-border dark:border-white/[0.07] bg-muted/40 dark:bg-white/[0.02]",
      )}>
        {/* Stage header */}
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            <span className={cn(
              ...
              task.status === "completed"
                ...
                : "bg-muted dark:bg-white/[0.05] text-muted-foreground border-border dark:border-white/[0.1]",
            )}>
        ...
        {findings.length > 0 && (
          <div className="mb-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {findings.map((f, i) => (
                <div key={i} className="rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] px-2.5 py-2">
...
function WorkOrderHeader({ wo }: { wo: WorkOrderSummary }) {
  ...
  const statusStyle = WO_STATUS_STYLES[wo.status] ?? "bg-muted dark:bg-white/[0.05] text-muted-foreground border-border dark:border-white/[0.1]";
  const mainBlockers = (wo.intelligence_packet?.blockers ?? []);

  return (
    <div className="border-b border-border dark:border-white/[0.07] px-5 py-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            {wo.assigned_minds.map((m) => (
              <span key={m} className="text-[10px] text-muted-foreground/60 bg-muted dark:bg-white/[0.04] rounded-full px-1.5 py-0.5 capitalize">
                {m}
              </span>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div className="rounded-lg border border-border dark:border-white/[0.06] bg-muted/40 dark:bg-white/[0.02] px-3 py-2.5">
      ...
      {/* Progress bar */}
      {wo.stage_count > 0 && (
        <div className="h-1.5 rounded-full bg-muted dark:bg-white/[0.06] overflow-hidden mb-4">
          <div
      ...
      {isLoading ? (
        ...
        <div className="px-5 py-10 text-center">
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-border dark:border-white/[0.08] bg-muted dark:bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground transition-all"
          >
