  ...
  return (
    <GrowthMindShell>
      <div className="min-h-screen p-6 space-y-6">
        ...
        {/* Filter bar */}
        {filtersOpen && (
          <div className="rounded-xl border border-border dark:border-white/[0.06] bg-card/50 p-4 flex flex-wrap gap-3 items-end">
            <div className="space-y-1">
              ...
              <select value={assetTypeFilter} onChange={e => setAssetTypeFilter(e.target.value as AssetType | "")}
                className="rounded-lg border border-border dark:border-white/[0.08] bg-muted/60 dark:bg-white/[0.03] px-3 py-1.5 text-xs text-foreground focus:outline-none">
            ...
            {activeFilters > 0 && (
              <button onClick={() => { setAssetTypeFilter(""); setPlatformFilter(""); setCampaignFilter(""); }}
                className="flex items-center gap-1 rounded-lg border border-border dark:border-white/[0.06] px-3 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
            <div className="flex gap-2">
              ...
              <a href={asset.image_url} target="_blank" rel="noopener noreferrer"
                className="flex h-7 w-7 items-center justify-center rounded-lg bg-muted dark:bg-white/10 hover:bg-muted dark:hover:bg-white/25 transition-colors" title="Full size">
            ...
            <div className="flex gap-1.5">
              <button onClick={onEdit}
                className="rounded border border-border dark:border-white/20 bg-muted dark:bg-white/10 px-2 py-1 text-[9px] hover:bg-muted dark:hover:bg-white/20 transition-colors">
                Edit
              </button>
          ...
          <div className="flex items-center gap-1 pt-0.5">
            <div className="relative flex-1">
              <button onClick={onToggleAttach}
                className="w-full rounded border border-border dark:border-white/[0.06] py-1 text-[9px] text-muted-foreground hover:text-orange-400 hover:border-orange-500/30 transition-colors">
                Attach
              </button>
              {attachOpen && (
                <div className="absolute bottom-full mb-1 left-0 z-30 w-52 rounded-lg border border-border dark:border-white/10 bg-popover shadow-xl overflow-hidden">
                  <p className="px-3 py-2 text-[10px] font-semibold text-muted-foreground border-b border-border dark:border-white/[0.06]">Attach to campaign</p>
                  {drafts.length === 0 ? (
                    ...
                    <div className="max-h-40 overflow-y-auto">
                      {drafts.map(d => (
                        <button key={d.id} onClick={() => onAttach(d.id)}
                          className="w-full px-3 py-2 text-left text-xs hover:bg-muted dark:hover:bg-white/[0.05] text-foreground transition-colors">
                          {d.name}
                        </button>
                      ))}
                    </div>
            ...
            <button onClick={onDelete} disabled={working}
              className="rounded border border-border dark:border-white/[0.06] px-1.5 py-1 text-muted-foreground hover:text-red-400 hover:border-red-500/30 transition-colors">
