    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        ...
        {activeConns.map(c => {
          ...
          return (
            <div key={c.id} className="rounded-xl border border-border dark:border-white/[0.06] bg-card/60 p-4 flex items-center gap-3">
              {c.profile_picture_url
                ? <img src={c.profile_picture_url} alt="" className="h-10 w-10 rounded-full object-cover" />
                : <div className="h-10 w-10 rounded-full bg-muted dark:bg-white/[0.06] flex items-center justify-center">
                ...
                {(c.permissions as any)?.length > 0 && (
                  <div className="flex items-center gap-1 mt-1 flex-wrap">
                    ...
                    {(c.permissions as string[]).map(p => (
                      <span key={p} className="text-[10px] px-1.5 py-0.5 rounded bg-muted dark:bg-white/[0.05] text-muted-foreground">{p}</span>
                    ))}
                  </div>
                )}
