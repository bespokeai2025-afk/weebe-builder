...
function PlatformKnowledgePage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 md:px-8 space-y-6">
      ...
      {/* Explanation */}
      <div className="rounded-xl border border-border bg-muted/40 p-5 text-xs text-muted-foreground leading-relaxed space-y-2">
...
function KbCard({ kb, stats }: { kb: any; stats: any }) {
  ...
  const meta = KB_META[kb.slug] ?? { icon: Library, accent: "text-foreground bg-muted ring-border", label: kb.slug };
  ...
  return (
    <div className="rounded-xl border border-border bg-muted/40">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center gap-3 p-4 text-left hover:bg-muted/40 transition-colors rounded-xl"
      >
...
function KbDetail({ kbId: _kbId, kbSlug }: { kbId: string; kbSlug: string }) {
  ...
  return (
    <div className="border-t border-border px-4 pb-4 pt-3 space-y-3">
      ...
      <div className="flex items-center gap-3">
        <label className={cn(
          "flex items-center gap-2 rounded-lg border border-border px-3 py-1.5 text-xs font-medium cursor-pointer transition-colors",
          uploading ? "opacity-60 cursor-not-allowed" : "hover:bg-muted",
        )}>
        ...
        <div className="space-y-1.5">
          {docs.map((doc: any) => {
            ...
            return (
              <div
                key={doc.id}
                className="flex items-center gap-3 rounded-lg border border-border/60 bg-muted/40 px-3 py-2 text-xs"
              >
