  ...
  return (
    <button onClick={onChange} className={cn(
      ...
      checked
        ...
        : "bg-muted/40 dark:bg-white/[0.02] text-muted-foreground border-border dark:border-white/[0.08] hover:text-foreground",
    )}>
...
function EmptyState({ icon: Icon, text }: { icon: React.ElementType; text: string }) {
  return (
    <div className="rounded-xl border border-dashed border-border dark:border-white/[0.06] px-4 py-4 flex items-center gap-3 text-muted-foreground">
...
function Dropdown<T extends string | number>({ value, options, onChange, renderLabel }: {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(p => !p)} className="flex items-center gap-1 rounded-md border border-border dark:border-white/[0.08] bg-muted/60 dark:bg-white/[0.03] px-2.5 py-1 text-xs font-medium text-foreground hover:bg-muted dark:hover:bg-white/[0.06]">
        {renderLabel(value)} <ChevronDown className="h-3 w-3" />
      </button>
      {open && (
        <div className="absolute left-0 top-8 z-30 min-w-[120px] rounded-xl border border-border dark:border-white/[0.08] bg-card shadow-xl overflow-hidden">
          {options.map((o, i) => (
            <button key={i} className={cn(
              "w-full px-3 py-2 text-left text-xs hover:bg-muted dark:hover:bg-white/[0.04] whitespace-nowrap",
      ...
      <div className="px-6 py-5 max-w-4xl space-y-5">
        ...
        {mode !== "observe" && sysRecs.length > 0 && (
          <div className="rounded-xl border border-violet-500/15 bg-violet-500/[0.03] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-border dark:border-white/[0.05]">
              <div className="flex items-center gap-2">
              ...
              {sysRecs.slice(0, 5).map(r => (
                <div key={r.id} className="flex items-start gap-3 px-4 py-3">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className="text-[9px] text-muted-foreground bg-muted dark:bg-white/[0.04] rounded px-1.5 py-0.5 hidden sm:block">
                      {r.category}
                    </span>
                    {r.action && (
            ...
            {sysRecs.length > 5 && (
              <div className="px-4 py-2 border-t border-border/60 dark:border-white/[0.04]">
                <Link
                  to="/hivemind/recommendations"
                  className="text-[11px] text-violet-400/60 hover:text-violet-300 transition-colors"
                >
        ...
        {/* CONFIGURE PANEL */}
        <div className="rounded-xl border border-border dark:border-white/[0.07] bg-card/50 overflow-hidden">
          <button
            className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-muted/40 dark:hover:bg-white/[0.02] transition-colors"
          ...
          {configOpen && (
            <div className="border-t border-border dark:border-white/[0.06] px-4 pb-4 pt-3 space-y-3">
              ...
              <div className="flex items-center gap-4 flex-wrap border-t border-border dark:border-white/[0.05] pt-2.5">
            ...
            {activeTab === "business" && (
              <div className="space-y-5">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {b.newBookings.map((bk: any) => (
                            <div key={bk.id} className={cn(
                              ...
                              {bk.notes && (
                                <p className="text-[11px] text-muted-foreground/70 mt-1.5 italic border-t border-border/60 dark:border-white/[0.04] pt-1.5 line-clamp-2">{bk.notes}</p>
                              )}
                            </div>
                          ))}
                        </div>
                          ...
                          {visibleStale.map((c: any) => (
                            <div key={c.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-muted/25 dark:hover:bg-white/[0.015] group">
                              ...
                              <button
                                ...
                                className="h-6 w-6 flex items-center justify-center rounded opacity-0 group-hover:opacity-100 hover:bg-muted dark:hover:bg-white/[0.06] transition-all shrink-0"
                              >
                ...
                {businessCount === 0 && !briefingQ.isLoading && (
                  mode !== "observe" && sysRecs.length > 0 ? (
                    <div className="rounded-xl border border-dashed border-border dark:border-white/[0.06] px-4 py-5 text-center space-y-1.5">
                      ...
                      <div key={r.id} className={cn(
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span className="text-[10px] text-muted-foreground bg-muted dark:bg-white/[0.04] rounded px-1.5 py-0.5">{r.category}</span>
                          </div>
