...
- Per-campaign breakdown: calls have no campaign_id column — standard attribution maps campaigns.agent_id → call.agent_id (most recently updated campaign wins per agent); WBAH reuses the dialler snapshot attribution (agent + nearest scheduled London slot). Campaign metrics live in the snapshot's metrics JSON alongside agents.
