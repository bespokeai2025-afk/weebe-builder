...
- E2E test recipe: signed webhook via HMAC-SHA256(body+ts, RETELL_API_KEY), header `x-retell-signature: v=<ts>,d=<hex>`. Note: Ava's live agent lives under the admin WORKSPACE Retell key (workspace_settings), not the platform key — sign test webhooks with that key.
