...
- Version pin only gates WEB calls: the phone path (triggerAvaCall) passes override_agent_id with no version pin and always uses the latest published version; AVA_AGENT_VERSION can silently drift stale (was 13 while live agent was at 17).
...
**Booking tool status is human-readable:** the deployed book_appointment_cal tool returns `{"status":"Successfully booked an appointment.","booking_id":N,"booking_uid":"..."}` — NOT the Cal v2 `status:"success"` literal. The verifier must accept any status starting with "success" (case-insensitive) or real bookings get flagged booking_failed and the Ads conversion is silently skipped (happened 2026-08-04). Genuine Cal 400 "already has booking at this time" = slot conflict, correctly flagged for follow-up.
