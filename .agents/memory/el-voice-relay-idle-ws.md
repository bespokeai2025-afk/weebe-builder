...
## Rule
The cascade voice relay (`src/lib/voice/gateway/cascade.gateway.ts`, formerly `el-voice-relay.plugin.ts`) must keep the WebSocket alive with a browser-side ping every 5 s.
...
2. **Server** (`src/lib/voice/gateway/cascade.gateway.ts`): `ping` message handler responds with `{type:"pong"}` via `safeSend` and returns — never passes through VAD.
