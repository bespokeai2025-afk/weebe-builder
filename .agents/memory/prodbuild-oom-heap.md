...
**How to apply:** If prodbuild FAILS with no error text and only one "built in" line (client done, SSR missing), suspect OOM first. Also note: background/nohup/setsid processes do NOT survive between agent bash calls here — run long builds via the workflow with a long `workflow_timeout`, not detached shells.
