...
**Pushing the other way (Replit → GitHub):** the workspace `.git` is sandbox-protected — even `git fetch` there is blocked. Clone the repo to /tmp (shallow), clear tracked files, tar-copy the workspace in (exclude .git/node_modules/dist/.cache/.local/.config/.upm/env files; rsync is NOT installed), `git add -A`, review staged changes for secrets, commit + plain push (never force). Auth: `GITHUB_PERSONAL_ACCESS_TOKEN` secret via a per-command `-c credential.helper` (user dismissed the GitHub OAuth connector; token never goes in URLs/config).
